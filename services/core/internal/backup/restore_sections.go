...
var restoreSectionPriority = map[string]int{
	...
	"forge_k_context_bundles":        76,
	"forge_k_context_snapshot_heads": 77,
	"model_manifests":                80,
...
var restoreExportOnlyReasons = map[string]string{
	...
	"forge_k_context_bundles":               "restore export-only by live policy: immutable Kernel context decisions require whole-store recovery verification",
	"forge_k_context_snapshot_heads":        "restore export-only by live policy: context decision heads are local CAS authority",
