...
func (s *Server) handleChatAssistantStream(w http.ResponseWriter, r *http.Request) {
	...
	if _, ok := s.modelRuntime.(modelRuntimeStreamingService); ok && !gateway.ShouldUseChatToolPipeline(umContent) && !s.nativeOllamaSelectedModelAvailable(ctx, ollamaAdapter, requestedModelID) {
		s.initSSE(w)
