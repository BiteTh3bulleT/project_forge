...
func forgeKAuthorityGateMatrixReadiness(validationReady bool) []ForgeKAuthorityGateMatrixEntry {
	...
	return []ForgeKAuthorityGateMatrixEntry{
		...
		},
		{
			...
			FeatureFlag:     "boot-selected forge_k authority; legacy_v1 COMPILE_CONTEXT fails closed",
			RollbackPath:    "select legacy_v1 to disable COMPILE_CONTEXT; existing immutable snapshots remain inspectable",
			...
			TestsPassing:    []string{"legacy_v1 fail-close tests", "persisted-compile idempotency tests", "proposal-source denial tests", "pure forgekernel/contextcompile tests", "restore-cache retirement tests"},
			Blockers:        []string{"Control Lane still computes packet/snapshot/restore decisions and reads legacy inputs", "pure forgekernel/contextcompile is not connected to live prepare/commit", "snapshot-head CAS and content-bound receipts remain incomplete"},
			OperatorVisible: true,
		},
		{
			Subsystem:       "KV System",
