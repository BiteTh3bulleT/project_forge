...
- [x] **Cross-platform OS integration readiness gate.** 2026-05-20: `npm run test:os-integration` covers gate regressions, and `npm run validate:os-integration` checks native desktop/operator VM package wiring, safe login defaults, loopback-only runtime defaults, local Ollama/modelruntime wiring, `/forge` durable storage roots, safe-mode shell authority flags, governed model-control runbook boundaries, CI/local validation wiring, and agent/runbook guidance on hosts where Nix is unavailable.
...
- [x] **Notification center surface.** 2026-05-20: added a shell notification center button and popover backed by `api.events(20)`, filtering operator-relevant job, approval, and notification events while leaving raw event history in the Activity log. Later the same day, added the Linux `org.freedesktop.Notifications` zbus backend in Tauri and bridged native app notifications into the same center through `forge://notification`.
- [x] **Host / hardware settings surface.** 2026-05-20: added read-only `GET /forge/system/host` backed by HostBridge diagnostics, plus a Settings page Host and Hardware section for host, CPU, memory, storage, GPU, display/audio/network/power status, session boundary, and config visibility. Mutation remains disabled; display apply waits for B5/C3 compositor support.
- [x] **Native app launcher discovery.** 2026-05-20: operator apps now keep the curated FORGE toolbelt first, then append visible XDG `.desktop` applications from system application directories with safe `Exec=` parsing, placeholder dropping, curated duplicate suppression, and shell/host-mutation launch rejection.
- [x] **Interim lock and OS-style power controls.** 2026-05-20: Start menu now exposes Lock, Logout, Restart, and Shutdown. Lock is a shell-owned FORGE re-auth overlay with the documented limitation that real session locking waits for compositor `ext-session-lock-v1`; Restart and Shutdown remain real host-power controls behind `FORGE_SHELL_DIRECT_SYSTEM_CONTROL`.
- [x] **Interim display layout intent.** 2026-05-20: Workspace Layouts now persists the operator's display arrangement preference (`preserve`, `extend`, or `mirror`), primary display, and detected display order. Output topology/resolution application remains deferred to compositor output-management support.
