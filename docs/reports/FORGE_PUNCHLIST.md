...
- [x] **Shell session restart control.** 2026-05-20: Start menu now exposes a confirmed Restart Shell action backed by Tauri `request_shell_session_action`, allowlisted to `restart_shell`, gated to `forge-shell-session`, and scoped to restarting the current desktop shell process without service control, host reboot, NixOS rebuild, modelruntime mutation, or semantic memory writes.
