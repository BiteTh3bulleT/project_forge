...
|---|---|---|---|
| `message.tool_calls[].function.name` | FORGE first selects exactly one tool through `gateway.SelectChatToolName` and exposes only that schema. `completeAssistantWithGatewayTools` rejects any different returned name before dispatch. | Proposal conformance only; FORGE already owns tool and lane selection. | Ambiguous turns expose no tool schema. Unknown or mismatched names fail before gateway dispatch. Tests: `TestOperationalLanguageDoesNotDelegateToolSelectionToModel`, `TestChatPostAmbiguousOperationalIntentNeverLetsModelSelectTool`, `TestDispatchRejectsUnknownModelFunctionBeforeGateway`, `TestChatPostForcedToolMismatchUsesDeterministicFallback`. |
...
- Models do not select tools. FORGE must deterministically select exactly one governed tool before exposing its proposal schema; otherwise the turn uses the no-tool path.
