...
| `FORGE_KERNEL_AUTHORITY_MODE` | [config.go](../../services/core/internal/config/config.go) + [forgekernel](../../services/core/internal/forgekernel/kernel.go) | `forge_k` | Selects exactly one semantic syscall ingress owner at boot. `forge_k` is the K20A default; `legacy_v1` is rollback only. There is no dual-write mode. |
