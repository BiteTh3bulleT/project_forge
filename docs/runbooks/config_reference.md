...
| `FORGE_ENABLE_METRICS_ENDPOINT` | [config.go](../../services/core/internal/config/config.go) + [metrics.go](../../services/core/internal/api/metrics.go) | `false` | Enables public `GET /metrics` Prometheus text exposition with bounded non-secret process/build/scrape metrics. When unset, `/metrics` is not mounted and returns 404. |
