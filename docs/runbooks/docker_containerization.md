...
The raw Compose file now defaults `FORGE_CORE_BIND_HOST=127.0.0.1` and `FORGE_ALLOW_WILDCARD_BIND=false`, matching direct Go, raw Docker image, and NixOS service fail-closed behavior. This is intentionally conservative: raw `docker compose up core` may be healthy inside the container while not exposing the core HTTP listener through the published host port.

Use the npm Docker helpers for the normal Docker-backed desktop workflow. `scripts/forge-docker-up.{sh,ps1}` generate or reuse `.forge/docker-api-token`, then explicitly set `FORGE_CORE_BIND_HOST=0.0.0.0` and `FORGE_ALLOW_WILDCARD_BIND=true` for the container-internal listener unless the operator or `.env.docker` already set those values. Host exposure is still controlled separately by `FORGE_DOCKER_BIND_HOST`, which defaults to `127.0.0.1`, and wildcard binds still require API auth: `/api/*`, `/forge/*`, and enabled `/v1/*` require `Authorization: Bearer <token>`, while `/health` remains public.

The standalone `services/core/Dockerfile` and the raw Compose defaults do not ship a wildcard-bind opt-in. The helper scripts are the explicit development-network profile for published Docker ports, local CORS for browser development, and token-backed API access.
