...
The Compose development stack keeps `FORGE_CORE_BIND_HOST=0.0.0.0` available for container port publishing, but it now defaults `FORGE_ALLOW_WILDCARD_BIND=false`. Operators must explicitly set both `FORGE_ALLOW_WILDCARD_BIND=true` and a non-empty `FORGE_API_TOKEN` when they intend to expose the core process from a container. That wildcard bind is still auth-gated: `/api/*`, `/forge/*`, and enabled `/v1/*` require `Authorization: Bearer <token>`, while `/health` remains public. Direct Go and NixOS service runs default to `127.0.0.1`, and wildcard binds fail closed unless explicitly opted in and an API token is available.
