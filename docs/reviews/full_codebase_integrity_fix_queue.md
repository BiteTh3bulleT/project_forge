...
Status: prioritized queue. CA1-001 through CA1-007, CA1-009, CA1-011, and CA1-012 through CA1-017 are closed by disabled-by-default host-power policy, root-workspace startup validation, fail-closed raw Compose bind defaults, corrected legacy adapter gateway/lane metadata, OS-login native shell posture, persisted Ollama URL validation, VM/Ollama docs alignment, desktop queue telemetry truth, route-detail shell coverage, lane default duplicate-ID coverage, desktop/Nix doc supersession, canonical VM evidence alignment, disabled-by-default debug-console handling, and shell registry drift tests; remaining rows stay queued until explicitly closed.
