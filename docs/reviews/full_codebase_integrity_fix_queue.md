...
Status: prioritized queue. CA1-001 through CA1-007 are closed by disabled-by-default host-power policy, root-workspace startup validation, fail-closed raw Compose bind defaults, corrected legacy adapter gateway metadata/gating, OS-login native shell posture, persisted Ollama URL validation, and VM/Ollama docs alignment; remaining rows stay queued until explicitly closed.
...
| CA1-004 | High | gateway/modelruntime boundary | `services/core/internal/api/legacy_adapter_gateway_tool.go`; `services/core/internal/api/server_adapters_test.go`; `services/core/internal/permissions/service.go` | Closed: `legacy.adapter.invoke` now advertises `scoped_execute` / `L2` / network use, remains gateway-only, and requires network/profile approval before adapter invocation. | fixed | yes |
