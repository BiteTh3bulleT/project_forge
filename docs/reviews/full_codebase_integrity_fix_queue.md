...
Status: prioritized queue. CA1-001 through CA1-004 are closed by disabled-by-default host-power policy, root-workspace startup validation, fail-closed raw Compose bind defaults, and corrected legacy adapter gateway metadata/gating; remaining rows stay queued until explicitly closed.
