...
Status: prioritized queue. CA1-001 through CA1-017 are closed by disabled-by-default host-power policy, root-workspace startup validation, fail-closed raw Compose bind defaults, corrected legacy adapter gateway/lane metadata, OS-login native shell posture, persisted Ollama URL validation, VM/Ollama docs alignment, structured JSON API error envelopes, desktop queue telemetry truth, shared operator app fallback catalog with Rust-id parity, route-detail shell coverage, lane default duplicate-ID coverage, desktop/Nix doc supersession, canonical VM evidence alignment, disabled-by-default debug-console handling, and shell registry drift tests.
...
| CA1-008 | Medium | API UX/errors | `services/core/internal/api/*`; `services/core/internal/api/error_response_static_test.go` | Closed: production API handlers now route former `http.Error` responses through structured JSON error helpers, internal build errors are logged behind generic 500 envelopes, and a static Go test rejects future raw `http.Error` use under `internal/api`. | fixed | no |
...
| CA1-010 | Medium | duplicate desktop catalog | `apps/desktop/src-tauri/src/main.rs`; `apps/desktop/src/lib/operatorApps.ts`; `apps/desktop/src/lib/operatorApps.test.ts`; `apps/desktop/src/layout/AppShellSurfaces.tsx`; `apps/desktop/src/pages/OperatorAppsPage.tsx` | Closed: Start and Operator Apps now share one TypeScript fallback catalog, the fallback covers the Rust allowlist ids, and Vitest compares fallback ids against the Rust `OPERATOR_APPS` block to detect drift. | fixed | no |
| CA1-011 | Medium | desktop route integration | `apps/desktop/src/App.tsx`; `apps/desktop/src/layout/AppShell.tsx`; `apps/desktop/src/layout/AppShell.test.tsx` | Closed: shell route-window behavior covers job detail and memory chunk detail deep links without creating orphan persistent windows; tests now cover both detail-route classes. | fixed | no |
