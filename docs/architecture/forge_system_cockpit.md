...
Status: DESIGN_ONLY / READ_ONLY_FIRST / PLANNED / NO_LIVE_AUTHORITY_CHANGE
...
|---|---|---|
| Core status | core URL, health, version/build marker, last refresh, degraded reasons. | READ_ONLY |
| FORGE-K activation readiness | validation seams, readiness blockers, authority non-change flags. | READ_ONLY |
| Authority gate matrix | gateway, approvals, audit, Control Lane, modelruntime, memory, host mutation, FORGE-K authority gates. | READ_ONLY |
...
| storage posture | SQLite default state, optional Postgres readiness, Qdrant shadow status, Redis ephemeral status. | READ_ONLY |
| Postgres/Qdrant/Redis readiness | disabled/default flags, DSN/endpoint configured state without secrets. | READ_ONLY |
...
A future implementation should extend the existing `GET /forge/system/status` read-only surface before adding new routes. If fields become too broad, split internal DTO construction while preserving a single bounded shell-facing response.
