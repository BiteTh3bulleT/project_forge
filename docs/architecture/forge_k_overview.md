...
Status: Current overview with historical simulator context and K20A-K20C production cutover. FORGE-K simulator services remain `SIMULATOR_ONLY`; the distinct production `internal/forgekernel` package owns semantic syscall ingress, durable stage orchestration, and deterministic Courthouse decisions while Control Lane implements the temporary validation/apply/SQLite port. Full FORGE-K authority is not complete.
