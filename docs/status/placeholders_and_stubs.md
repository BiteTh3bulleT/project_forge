...
| `services/core/internal/api/server.go` memory mutation handlers | v1 observation mutation endpoints can bypass syscall semantics if left open | production risk (legacy boundary) | guarded | Mutation endpoints are now default-blocked unless `FORGE_ALLOW_LEGACY_MEMORY_MUTATIONS=true`; blocked/used attempts are audited. |
...
| `services/core/internal/gateway/tool_capability_registry.go` | Full taxonomy, majority stubbed/deferred by design | safe scaffold | yes | Preserved intentionally; status/policy makes executable posture explicit. |
