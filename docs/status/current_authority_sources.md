...
Current Context Compiler-related live work remains validation/shadow only. K20I adds the isolated pure production contract `services/core/internal/forgekernel/contextcompile`, which deterministically binds exact-scope admitted K20H sources, immutable snapshot candidates, governed outcome heads, fixed-point restore policy, and complete output commitments; it is not yet connected to live `COMPILE_CONTEXT`. `VALIDATE_CONTEXT_ATTRIBUTION` and existing disabled-by-default `forgekshadow` diagnostics remain non-authoritative. No current claim should treat the pure contract or simulator `services/core/internal/forgek/contextcompiler` as live prompt authority.
