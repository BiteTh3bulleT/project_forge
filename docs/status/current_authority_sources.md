...
| Model runtime | `services/core/internal/forgekernel/runtimeproposal`, `services/core/internal/modelruntime`, `services/core/internal/api/model_runtime*.go` | Models are proposal-only drivers. Every response buffers model bytes until a pure hash-bound runtime decision and consensus gate. Gateway completion claims require actual invocation/result/audit evidence, and every model prompt binds a live Kernel Context Compiler decision. |
| Context Compiler | `services/core/internal/forgekernel/contextcompile`, production Kernel, bounded Control Lane gather/SQLite port | K20J fixed-point compilation binds exact scope, current admitted memory evidence, governed candidates, the prior CAS head, policy, and every output commitment. The Kernel decides; one transaction persists immutable bundle/head and normal commit/replay proof. Legacy notes and snapshots cannot enter model prompts. |
...
Current Context Compiler authority is production-owned. `COMPILE_CONTEXT` requires production FORGE-K, idempotency, exact nonempty scope, and authenticated authorization. The pure `services/core/internal/forgekernel/contextcompile` contract binds current admitted K20H evidence, governed candidates, the exact prior CAS head, fixed-point policy, and complete packet/snapshot/selection commitments. The durable port gathers typed rows and revalidates them inside the transaction; it cannot choose another decision. Immutable bundle/head, journal, authorization, audit-outbox, and replay proof commit atomically. Every live model prompt uses this decision and excludes legacy notes, chat-history memory, and legacy snapshots from authority.

Current Runtime Boundary authority is the pure production `services/core/internal/forgekernel/runtimeproposal` contract plus the live API coordinator. Ordinary chat, native Ollama plain/stream, modelruntime plain/stream, gateway tool-loop synthesis, `/forge/models/{id}/chat`, and `/v1/chat/completions` buffer model output until its exact bytes, driver identity, scope, provenance, live Kernel context decision/bundle commitment, and any gateway evidence are classified. Unsupported authority or unbound action-completion claims are replaced with fixed safe text. Streamed stage events expose commitments rather than raw model JSON/arguments.
