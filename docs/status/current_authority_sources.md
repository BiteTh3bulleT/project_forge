...
| Memory acceleration projection | production Kernel, `services/core/internal/memory/vsaprojection`, `services/core/internal/aios/controllane/sqlite_store_acceleration.go` | K20G owns exact-scope deterministic VSA manifest rebuild and atomic head swap. Legacy/unscoped rows are ignored. Activation remains fail-closed until a production syscall creates admitted scoped memory sources. |
