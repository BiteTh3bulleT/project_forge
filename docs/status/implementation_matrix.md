...
| Model runtime governance (M3/M4) | `/forge/models*` + `modelruntime.Service` (with gated `/v1/*` compatibility) | partial (implemented) | `services/core/internal/modelruntime/*`, `services/core/internal/api/model_runtime.go`, `services/core/internal/api/model_runtime_bridge.go`, `services/core/internal/config/config.go` | M4 items remain: delete-file approval flow, stronger backend/process supervision, gateway `model.*` aliasing, deeper multi-backend scheduling, streaming hardening beyond chat/SSE |
...
4. Model runtime M3/M4 is now real: manifest/store/registry/backend/runtime service/APIs/config/audit paths are implemented with import/register flows, persistent lifecycle state, deterministic selection, backend expansion, runtime inspection endpoints, and governed chat SSE streaming; remaining M4 items stay explicit.
