...
| Audit history (`audit_records`, `gateway_invocations`) | yes (limited extract window) | yes (same extracted rows) | parity within export window |
| VSA tables (`memory_vsa_*`, `retrieval_result_vsa_signals`) | yes | no | export-only by explicit restore policy |

## Claim boundary

FORGE can now claim **material high-value restore parity** for approvals/events/jobs/artifacts, project context/evaluations, audit/gateway history, and core cognitive/autonomy sections.
