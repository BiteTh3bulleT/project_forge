...
| Semantic memory/state mutation | production `forgekernel.Kernel` plus one K-owned `DurablePort` implemented by Control Lane | read-only observation inspection APIs (`/api/memory/*` GET); offline recovery only | K20J single-authority cutover | low; no live alternate remains | Mutation endpoints remain retired (`410 Gone`); daemon bootstrap constructs one port and one Kernel orchestrator; no fallback or dual commit. |
