...
| Semantic memory/state mutation | v2 syscall kernel (`aios/controllane`) | v1 memory observation APIs (`/api/memory/*`) | duplicated | truth/path drift | Wrap or explicitly boundary-label v1 APIs as legacy until migrated. |
...
3. Converge v1 memory mutation routes behind syscall kernel or explicit legacy boundary controls.
