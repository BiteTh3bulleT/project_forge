...
| Multiple native windows remain separate | partial | Implementation keys by compositor window id | Manual two-terminal/two-file-window smoke. |
| Closed native apps disappear from taskbar | partial | Backend registry design and existing placeholder resolution test | Frontend stale-disappearance regression/manual smoke. |
...
Result: focused AppShell tests passed 30 tests after the G8 native lifecycle fixes; desktop validation passed 102 frontend tests plus build/typecheck; root core tests passed; core vet passed; `git diff --check` passed. The red run before implementation failed the five expected new native lifecycle/action cases.
