...
Updated: 2026-05-12 (cleanup placeholder filtering reality alignment)
...
| `services/core/internal/aios/compute/librarian/cells_phase4.go` | `CleanupRuntimeCell.Run` | Filters placeholder/fake archive targets locally before proposing `ARCHIVE_NOTE`; downstream autonomy commit validation remains an additional guard. Covered by `TestCleanupRuntimeCellSkipsPlaceholderArchiveTargets`. | resolved hardening | low | keep |
...
- `candidate-*`/`fake-*` placeholder IDs were found in runtime as explicit blocklist guards in autonomy commit validation and as cleanup-cell pre-proposal filters.
...
1. Keep direct adapter invoke route removed; do not reintroduce a non-gateway adapter execution ingress.
2. Fail closed for autonomy/control-lane durable paths in production wiring when DB/transaction backing is absent, rather than silently falling back to in-memory repositories.
3. Keep cleanup placeholder filtering covered when archive proposal behavior changes.
