...
- K20A (2026-08-14) satisfies that requirement for the first production boundary. The distinct `services/core/internal/forgekernel` package owns semantic syscall ingress by default, with `legacy_v1` as rollback-only. K20B makes that production Kernel own prepare/commit/audit/observe orchestration through a narrow durable port implemented temporarily by `aios/controllane`. These amendments do not promote the in-memory simulator under `services/core/internal/forgek`; all simulator-isolation rules in this ADR remain active. See ADR 0017.
