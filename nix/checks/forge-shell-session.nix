...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    grep -F 'FORGE_DATA_DIR=''${config.services.forge-core.dataDir}' "$module"
    grep -F 'FORGE_API_TOKEN_FILE=''${config.services.forge-core.dataDir}/auth/api_token' "$module"

    grep -F 'fn fit_operator_desktop_window' "$desktop_main"
    grep -F 'fn fullscreen_shell_enabled' "$desktop_main"
    grep -F 'window.set_decorations(false)?;' "$desktop_main"
    grep -F 'window.set_resizable(false)?;' "$desktop_main"
    grep -F 'window.set_fullscreen(true)?;' "$desktop_main"
