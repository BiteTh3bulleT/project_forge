...
runCommand "forge-operator-vm-check"
  ...
  ''
    ...
    native_profile="$src/nix/nixos/profiles/forge-native-desktop-runtime.nix"
    ...
    grep -F 'forge-operator-desktop-shell' "$flake"
    grep -F 'forge-operator-shell-session' "$flake"
    grep -F 'renderProfile = "vm-safe";' "$flake"
    grep -F 'bootLogin = false;' "$flake"
    grep -F 'emptyDesktopOnBoot = true;' "$flake"
    grep -F 'forge-operator-desktop-shell' "$src/nix/overlays/default.nix"
    grep -F 'forge-operator-shell-session' "$src/nix/overlays/default.nix"
    ...
    grep -F 'boot.loader.timeout = lib.mkDefault 0;' "$config"
    grep -F 'fileSystems."/" = {' "$config"
    grep -F 'boot.plymouth = {' "$config"
    grep -F 'theme = lib.mkForce "forge";' "$config"
    grep -F 'ShowDelay=0' "$config"
    ...
    grep -F 'services.ollama = {' "$config"
    grep -F 'home = lib.mkDefault "/forge/models/ollama";' "$config"
    grep -F 'host = lib.mkDefault "127.0.0.1";' "$config"
    grep -F 'port = lib.mkDefault 11434;' "$config"
    ...
    grep -F 'FORGE_BOOT_SPLASH=plymouth' "$config"
    grep -F 'FORGE_BOOT_LOGIN=greetd-regreet' "$config"
    grep -F 'unset FORGE_SHELL_BINARY' "$src/nix/packages/forge-operator-session.nix"
    grep -F 'virtualisation.vmVariant' "$config"
    grep -F 'services.greetd.enable == true' "$config"
    if grep -F 'initial_session' "$config"; then
      echo "canonical operator VM must not configure greetd initial_session autologin" >&2
      exit 1
    fi

    grep -F './forge-operator-desktop.nix' "$native_profile"
    grep -F 'bootLogin = false;' "$native_profile"
    grep -F 'emptyDesktopOnBoot = true;' "$native_profile"
    grep -F 'programs.regreet = {' "$native_profile"
    grep -F 'services.greetd = {' "$native_profile"
    grep -F 'services.getty.autologinUser = lib.mkForce null;' "$native_profile"
    grep -F 'defaultSession = lib.mkDefault "forge-operator";' "$native_profile"
    grep -F 'sessionPackages = [ forgeOperatorSession ];' "$native_profile"
    grep -F 'FORGE_NATIVE_DESKTOP_DEFAULT_SESSION=forge-operator' "$native_profile"
    grep -F 'FORGE_NATIVE_DESKTOP_RUNTIME=true' "$native_profile"
