...
runCommand "forge-operator-vm-check"
  ...
  ''
    ...
    grep -F 'theme = lib.mkDefault "forge";' "$config"
    ...
    grep -F 'services.greetd = {' "$config"
    grep -F 'command = forgeOperatorSessionCommand;' "$config"
    ...
    grep -F 'FORGE_BOOT_LOGIN=forge-greeter' "$config"
