...
runCommand "forge-operator-toolbelt-check" { } ''
  ...
  for bin in \
    foot pcmanfm mousepad firefox ollama btop htop sqlitebrowser jq yq rg fd bat eza tree git lazygit \
    curl wget nmap dig ip ss lspci lsusb lsof strace micro hx xarchiver \
    forge-operator-btop forge-operator-lazygit forge-operator-ollama-status \
