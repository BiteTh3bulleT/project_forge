...
runCommand "forge-operator-desktop-profile-check"
  ...
  ''
    ...
    grep -F 'forge-operator-desktop-shell' "$src/flake.nix"
    grep -F 'forge-operator-shell-session' "$src/flake.nix"
    grep -F 'renderProfile = "vm-safe";' "$src/flake.nix"
    grep -F 'bootLogin = false;' "$src/flake.nix"
    grep -F 'emptyDesktopOnBoot = true;' "$src/flake.nix"
