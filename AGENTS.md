...
Last convergence sweep update: 2026-08-14 (K20A production authority alignment).
...
- `[LIVE / SOLE AUTHORITY]` `services/core/internal/forgekernel` is the production FORGE-K boundary. It owns authenticated ingress, durable stage order, deterministic decisions, sealed commit/receipt/replay, authorization proof, Context Compiler decisions, and model-output visibility. `aios/controllane` is a bounded validation/apply/SQLite durable port.
...
- `[LIVE / BOUNDED DRIVERS]` Gateway, model runtime, retrieval, embeddings, memory, API, and Control Lane remain external drivers/ports. They cannot claim Kernel, truth, admission, journal, or model-visibility authority.
- `[LIVE]` Memory repair is proposal-only; retrieval evidence, utility evidence, admitted memory, and VSA rebuilds use explicit Kernel contracts. Legacy observation writers remain retired.
...
- Treat production FORGE-K as the sole live authority while preserving explicit limits: unimplemented capabilities remain fail-closed, external audit delivery is a projection, and recovery is daemon-stopped/whole-store.

## Project Priorities
- **2026-08-17**: Successful FORGE-K commits deliver their atomic audit intent through a restart-safe idempotent projector. Exact request/authorization/receipt/journal proof is reverified before sink delivery; immutable delivery/retry/quarantine attempts preserve outages and poison evidence. Legacy semantic-row `audit_id` backfill is retired in favor of immutable outbox-to-audit projection identity.
- **2026-08-17**: The isolated physical OptiPlex profile runs in explicit full-authority test mode: production FORGE-K remains the only semantic/model-visibility authority while the operator shell enables implemented host-power, model-lifecycle, semantic-write, accelerator, and bounded autonomous-maintenance controls. The direct-link nftables egress deny remains mandatory; a model or shell flag never grants an alternate truth or Kernel authority path.
- **2026-08-16**: The physical OptiPlex profile is a reproducible full offline development workstation: office/document/graphics/media applications, VSCodium with pinned extensions, the complete FORGE language/toolchain set, rootless containers, backup clients, audio/printing/scanning/removable-media services, and a group-writable bounded FORGE workspace. This host provisioning does not grant applications or Nix live semantic authority.
...
- **2026-08-16**: K20J completes the live Context Compiler and runtime visibility cutover. Model prompts bind immutable exact-scope Kernel context decisions; all model output is withheld behind Kernel runtime-proposal and consensus decisions. Tokenless remote HTTP fails closed.
...
- **2026-08-14 (superseded by K20J)**: K20B moved live syscall stage orchestration into production FORGE-K. The K-owned durable port separates preflight, one atomic apply+journal commit, audit recording, and best-effort observation. K20J subsequently removed the `legacy_v1` boot facade; Control Lane remains only the bounded policy/apply/SQLite port.
- **2026-08-14 (superseded by K20J)**: K20A began the production FORGE-K cutover with a temporary rollback selector. K20J removed that selector and made `internal/forgekernel` the sole production semantic authority. FORGE selects the exact chat tool before any model proposal call; ambiguous turns expose no tool schema.
...
- **2026-08-14**: K20C makes production `services/core/internal/forgekernel/court` the live deterministic owner for `ADMIT_EVIDENCE` and `APPEAL_RULING`. Current exhibit state, immutable ruling/appeal history, provenance, and the semantic journal event commit atomically; simulator Court remains isolated and model/proposal/`legacy_v1` ruling paths fail closed. Full FORGE-K authority remains incomplete pending K20D atomic receipt/audit/idempotency/journal-chain integrity and later subsystem cutovers.
