...
- **2026-05-08**: Phase G3 adds the manual `forge-desktop-shell` Nix package/app surface and wires `forge-shell-session` to prefer it after it contains the real Tauri binary. The package is launcher-only until full Tauri Nix packaging lands; it still does not autostart, replace the desktop, install or require a compositor, mutate host state, run service control, change modelruntime behavior, write semantic memory, or make FORGE-K live authority.
