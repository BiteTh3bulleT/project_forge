...
export function HumanDataView({
  ...
  return (
    <div className={compact ? "space-y-1" : "space-y-2"}>
      {renderValue(value, 0, maxDepth, compact)}
    </div>
  );
}
...
function renderValue(
  ...
  if (Array.isArray(value)) {
    if (value.length === 0)
      return <span className="text-forge-mist/75">No items</span>;
    return (
      <div className={compact ? "space-y-1" : "space-y-2"}>
        {value.map((item, index) => (
          ...
          >
            <div className="mb-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-forge-mist/60">
              Item {index + 1}
            </div>
  ...
  if (typeof value === "object" && value !== null) {
    ...
    if (entries.length === 0)
      return <span className="text-forge-mist/75">No details</span>;
    ...
    return (
      <div className={compact ? "grid gap-1" : "grid gap-2"}>
        {entries.map(([key, item]) => (
          ...
          >
            <div className="text-[10px] font-semibold uppercase tracking-[0.12em] text-forge-mist/65">
              {humanizeKey(key)}
            </div>
