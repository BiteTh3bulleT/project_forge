...
export function ActionLanesPage() {
  ...
  return (
    <div className="forge-ops-board space-y-5">
      ...
      <Panel
        title="Add custom lane"
        subtitle="Non-builtin id — used for gateway + policy. Paths are usually absolute or workspace roots from core meta."
      >
        <div className="grid gap-3 md:grid-cols-2">
          <label className="text-xs text-forge-mist">
            ...
            <input
              ...
              value={newLane.id}
              onChange={(e) =>
                setNewLane((n) => ({ ...n, id: e.target.value }))
              }
            />
          </label>
          <label className="text-xs text-forge-mist">
            ...
            <input
              ...
              value={newLane.name}
              onChange={(e) =>
                setNewLane((n) => ({ ...n, name: e.target.value }))
              }
            />
          </label>
          <label className="text-xs text-forge-mist md:col-span-2">
            ...
            <input
              ...
              onChange={(e) =>
                setNewLane((n) => ({ ...n, description: e.target.value }))
              }
            />
          </label>
          <label className="text-xs text-forge-mist">
            ...
            <input
              className="forge-input mt-1 font-mono text-xs"
              value={newLane.actionType}
              onChange={(e) =>
                setNewLane((n) => ({ ...n, actionType: e.target.value }))
              }
            />
          </label>
          <label className="text-xs text-forge-mist">
            ...
            <input
              className="forge-input mt-1"
              value={newLane.riskClass}
              onChange={(e) =>
                setNewLane((n) => ({ ...n, riskClass: e.target.value }))
              }
            />
          </label>
          <label className="text-xs text-forge-mist md:col-span-2">
            ...
            <textarea
              ...
              value={newLane.allowedPaths}
              onChange={(e) =>
                setNewLane((n) => ({ ...n, allowedPaths: e.target.value }))
              }
            />
          </label>
          <label className="text-xs text-forge-mist md:col-span-2">
            ...
            <textarea
              ...
              value={newLane.forbiddenPaths}
              onChange={(e) =>
                setNewLane((n) => ({ ...n, forbiddenPaths: e.target.value }))
              }
            />
          </label>
          <label className="text-xs text-forge-mist md:col-span-2">
            ...
            <textarea
              className="forge-input mt-1 min-h-[48px] font-mono text-[11px]"
              value={newLane.expectedArtifacts}
              onChange={(e) =>
                setNewLane((n) => ({ ...n, expectedArtifacts: e.target.value }))
              }
            />
          </label>
          <label className="text-xs text-forge-mist">
            Max bytes
            <input
              className="forge-input mt-1 font-mono"
              value={newLane.maxBytes}
              onChange={(e) =>
                setNewLane((n) => ({ ...n, maxBytes: e.target.value }))
              }
            />
          </label>
          <div className="flex flex-col gap-2 text-xs text-forge-mist md:pt-6">
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={newLane.writeIntent}
                onChange={(e) =>
                  setNewLane((n) => ({ ...n, writeIntent: e.target.checked }))
                }
              />
              Write intent
            </label>
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={newLane.requiresApproval}
                onChange={(e) =>
                  setNewLane((n) => ({
                    ...n,
        ...
        <div className="mt-3">
          <PrimaryButton onClick={() => void createCustomLane()}>
            Create lane
          </PrimaryButton>
        </div>
      </Panel>
      ...
      <div className="space-y-3">
        {lanes.map((ln) => (
          <div
            key={ln.id}
            className="rounded-lg border border-white/10 bg-forge-iron/40 p-4"
          >
              ...
              <div className="flex flex-wrap items-center gap-2">
                <label className="flex items-center gap-2 text-[11px] text-forge-mist">
                  <input
                    type="checkbox"
                    checked={ln.enabled}
                    onChange={() => void toggleEnabled(ln)}
                  />
                  Enabled
                </label>
                <span className="text-[11px] text-forge-mist/70">
                  updated {formatTime(ln.updatedAtMs)}
                </span>
              </div>
            </div>
            ...
            <div className="mt-2 flex flex-wrap gap-2 text-[11px] text-forge-mist">
              ...
              <span className="rounded border border-white/10 px-2 py-0.5">
                {ln.writeIntent ? "write-intent" : "read-only"}
              </span>
              <span className="rounded border border-white/10 px-2 py-0.5">
                {ln.requiresApproval ? "approval gate" : "no lane approval"}
              </span>
              <span className="rounded border border-white/10 px-2 py-0.5">
                {ln.builtin ? "builtin" : "custom"}
              </span>
              <span className="rounded border border-white/10 px-2 py-0.5">
                maxBytes {ln.maxBytes}
              </span>
            ...
            <div className="mt-3 grid gap-2 md:grid-cols-2">
              <div className="rounded border border-white/10 bg-black/20 p-2">
                <div className="text-[10px] font-semibold text-forge-ash">
                  Allowed paths
                </div>
              ...
              <div className="rounded border border-white/10 bg-black/20 p-2">
                <div className="text-[10px] font-semibold text-forge-ash">
                  Forbidden paths / expected artifacts
                </div>
                <div className="mt-1 max-h-28 overflow-auto text-[10px] text-forge-mist">
                  <HumanDataView
                    value={{
            ...
            {!ln.builtin ? (
              <div className="mt-3 flex flex-wrap gap-2">
                <GhostButton
                  onClick={() => {
                    const allowed = window.prompt(
                      "Allowed paths (one path per line)",
                      pathsToText(ln.allowedPaths),
                    );
                    if (allowed == null) return;
                    const forbidden = window.prompt(
                      "Forbidden paths (one path per line)",
                      pathsToText(ln.forbiddenPaths),
                    );
                ...
                <GhostButton onClick={() => void deleteLane(ln)}>
                  Delete lane
                </GhostButton>
              </div>
            ) : (
              <p className="mt-3 text-[11px] text-forge-mist/55">
                Built-in scope is fixed in core; only Enabled can be toggled
                here.
              </p>
            )}
          </div>
