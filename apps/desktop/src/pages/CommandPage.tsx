    ...
    <div className="forge-ops-board space-y-5">
      ...
      >
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <Quick
            title="Chat"
            body="Persisted operator threads; optional Ollama replies; queue jobs from a thread."
            onClick={() => navigate("/chat")}
          />
          <Quick
            title="Workbench"
            body="Artifact index, textual preview, optional job correlation."
            onClick={() => navigate("/workbench")}
          />
          <Quick
            title="Canvas"
            body="Scratch boards with positioned notes (SQLite-backed)."
            onClick={() => navigate("/canvas")}
          />
          <Quick
            title="Dashboard"
            body="View live queues, failures, reviews, and routing advisories."
            onClick={() => navigate("/dashboard")}
          />
          <Quick
            title="Policy"
            body="Inspect presets, recommendations, and packet guidance."
            onClick={() => navigate("/policy")}
          />
          <Quick
            title="Jobs"
            body="Inspect lifecycle, logs, packet contract, and artifacts."
            onClick={() => navigate("/jobs")}
          />
          <Quick
            title="Approvals"
            body="Review pending high-risk requests and decide."
            onClick={() => navigate("/approvals")}
          />
          <Quick
            title="Strategies"
            body="Tune reusable strategy contracts for common task types."
            onClick={() => navigate("/strategies")}
          />
          <Quick
            title="Automation"
            body="Run bounded rules with dry-run previews and history."
            onClick={() => navigate("/automation")}
          />
          <Quick
            title="Reviews"
            body="Resolve imported/output review queue with persisted decisions."
            onClick={() => navigate("/reviews")}
          />
          <Quick
            title="Project Context"
            body="Import and normalize FORGE context into durable guidance files."
            onClick={() => navigate("/project-context")}
          />
          <Quick
            title="Memory"
            body="Search indexed retrieval context and open chunk detail."
      ...
      <Panel
        title="Template Jobs"
        subtitle="Each launch maps to a controlled template with explicit risk class and artifact expectations."
      >
