...
export function ReleasePage() {
  ...
  const [firstRun, setFirstRun] = useState<Record<string, unknown> | null>(
    null,
  );
  ...
  return (
    <div className="forge-ops-board space-y-5">
      <Panel
        ...
        subtitle="Local packaging gate: filesystem, migrations, lanes, permissions, first-run."
        actions={
      ...
      >
        ...
        <div className="mt-2 text-sm text-forge-mist">
          Overall:{" "}
          <span className={ready ? "text-forge-ash" : "text-forge-emberSoft"}>
            {ready ? "READY (reported)" : "NOT READY"}
          </span>
        </div>
      </Panel>
      ...
      {firstRun ? (
        <Panel
          ...
          subtitle="Operator onboarding state from the core."
        >
      ...
      <Panel
        ...
        subtitle="Each item is independently inspectable."
      >
        <div className="space-y-2">
          {items.map((it) => (
            <div
              key={it.id}
              className="rounded border border-white/10 bg-forge-iron/30 p-3 text-xs text-forge-mist"
            >
              ...
              <div className="mt-1 text-[10px] uppercase tracking-wide text-forge-mist/70">
                {it.category}
              </div>
            </div>
          ))}
      ...
      <Panel
        title="Record release artifact"
        subtitle="Use after running your local packaging script (e.g. tauri build); this is bookkeeping, not the build itself."
      >
        <div className="grid gap-2 md:grid-cols-2">
          ...
          <input
            className="forge-input"
            value={recVersion}
            onChange={(e) => setRecVersion(e.target.value)}
            placeholder="version tag"
          />
        </div>
        <textarea
          className="forge-input mt-3 min-h-[72px]"
          value={recSummary}
      ...
      <Panel
        ...
        subtitle="release_artifacts table — audit-friendly packaging history."
      >
        <div className="space-y-2">
          {artifacts.map((row, idx) => (
            <div
              key={idx}
              className="max-h-36 overflow-auto rounded border border-white/10 bg-black/25 p-2 text-[11px] text-forge-mist"
            >
