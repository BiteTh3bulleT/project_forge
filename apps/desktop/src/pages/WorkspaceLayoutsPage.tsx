...
export function WorkspaceLayoutsPage() {
  ...
  const removeLayoutWindow = useWorkspaceLayoutStore((s) => s.removeLayoutWindow);
  const updateLayoutWindow = useWorkspaceLayoutStore((s) => s.updateLayoutWindow);
    ...
    <div className="space-y-6">
      <Panel title="Workspace Layouts" subtitle="Monitor-aware multi-window presets backed by real display detection and Tauri window placement.">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          <MetricCard label="Detected displays" value={String(monitors.length)} detail={monitors.length > 0 ? monitors.map((m) => m.name ?? `Display ${m.ordinal + 1}`).join(" · ") : "No desktop monitor API available."} />
          <MetricCard label="Saved layouts" value={String(layouts.length)} detail={activeLayoutId ? `Active: ${layouts.find((layout) => layout.id === activeLayoutId)?.name ?? activeLayoutId}` : "No active layout."} />
      ...
      <div className="grid gap-6 xl:grid-cols-[320px_minmax(0,1fr)]">
        <Panel title="Preset List" subtitle="Create, select, duplicate, rename, delete, and activate named workspace layouts.">
          <div className="mb-3 flex gap-2">
            <button type="button" className="forge-btn forge-btn--primary" onClick={() => createLayout(`Layout ${layouts.length + 1}`)}>
              New layout
            </button>
          </div>
          ...
          <div className="space-y-6">
            <Panel
              ...
              actions={
                <div className="flex flex-wrap gap-2">
                  <button type="button" className="forge-btn forge-btn--ghost" onClick={() => duplicateLayout(selectedLayout.id)}>
                    Duplicate
                  </button>
                  <button type="button" className="forge-btn forge-btn--ghost" onClick={() => void captureRuntimeIntoLayout(selectedLayout.id)}>
                    Capture running windows
                  </button>
                  <button type="button" className="forge-btn forge-btn--primary" onClick={() => void activateLayout(selectedLayout.id)}>
                    Activate layout
                  </button>
                </div>
              }
            >
              <label className="block text-xs text-forge-mist">
                Layout name
                <input className="forge-input mt-1" value={selectedLayout.name} onChange={(e) => renameLayout(selectedLayout.id, e.target.value)} />
              </label>
                  ...
                  <div key={windowRecord.id} className="rounded-xl border border-white/10 bg-black/20 p-4">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div>
                        ...
                        <div className="mt-1 text-xs text-forge-mist">Runtime label: <span className="font-mono">{windowRecord.runtimeLabel}</span></div>
                      </div>
                      <div className="flex gap-2">
                        <button type="button" className="forge-btn forge-btn--ghost" onClick={() => addLayoutWindow(selectedLayout.id)}>
                          Add window
                        </button>
                        {windowRecord.runtimeLabel !== "main" ? (
                          <button type="button" className="forge-btn forge-btn--ghost" onClick={() => removeLayoutWindow(selectedLayout.id, windowRecord.id)}>
                            Remove
                          </button>
                        ) : null}
                      </div>
                    ...
                    <div className="mt-4 grid gap-3 md:grid-cols-2">
                      <label className="text-xs text-forge-mist">
                        Title
                        <input className="forge-input mt-1" value={windowRecord.title} onChange={(e) => updateLayoutWindow(selectedLayout.id, windowRecord.id, { title: e.target.value })} />
                      </label>
                      <label className="text-xs text-forge-mist">
                        Role
                        <select className="forge-input mt-1" value={windowRecord.role} onChange={(e) => updateLayoutWindow(selectedLayout.id, windowRecord.id, { role: e.target.value as never })}>
                      ...
                      <label className="text-xs text-forge-mist">
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            const monitor = monitors.find((item) => item.id === raw) ?? null;
                            updateLayoutWindow(selectedLayout.id, windowRecord.id, {
                              targetMonitorId: monitor?.id ?? null,
                              targetMonitorOrdinal: monitor?.ordinal ?? (Number(raw.replace("ordinal:", "")) || 0),
                            });
                          }}
                        >
                          {monitors.length === 0 ? <option value={`ordinal:${windowRecord.targetMonitorOrdinal}`}>No live display list</option> : null}
                      ...
                      <label className="text-xs text-forge-mist">
                        Default surface
                        <select className="forge-input mt-1" value={windowRecord.activeRoute} onChange={(e) => updateLayoutWindow(selectedLayout.id, windowRecord.id, { activeRoute: e.target.value })}>
                          {windowRecord.assignedRoutes.map((route) => (
                            <option key={route} value={route}>
                              {assignableShellTools.find((tool) => tool.route === route)?.label ?? route}
                            </option>
                          ))}
                        </select>
                      </label>
                    ...
                    <div className="mt-4">
                      <div className="text-xs font-semibold tracking-[0.16em] text-forge-mist">Assigned surfaces</div>
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  const nextRoutes = e.target.checked
                                    ? [...windowRecord.assignedRoutes, tool.route]
                                    : windowRecord.assignedRoutes.filter((route) => route !== tool.route);
                                  updateLayoutWindow(selectedLayout.id, windowRecord.id, { assignedRoutes: nextRoutes });
                                }}
                              />
                              <span>
                                <span className="block font-semibold text-forge-ash">{tool.label}</span>
                                <span className="block text-[11px] leading-relaxed text-forge-mist">{tool.description}</span>
                              </span>
                            </label>
                          );
                        })}
                    ...
                    {windowRecord.fallbackReason ? <div className="mt-3 text-xs text-forge-mist">Last fallback: {windowRecord.fallbackReason}</div> : null}
                    {index === 0 ? <div className="mt-3 text-xs text-forge-mist">The first window is pinned to the real Tauri `main` shell so restore always has a recovery anchor.</div> : null}
                  </div>
                ))}
              </div>
              <div className="mt-4 flex gap-2">
                <button type="button" className="forge-btn forge-btn--ghost" onClick={() => void deleteLayout(selectedLayout.id)} disabled={layouts.length <= 1}>
                  Delete layout
                </button>
              </div>
            </Panel>

            <Panel title="Runtime Windows" subtitle="Real shell windows currently registered by the desktop runtime.">
              <div className="space-y-2">
                {runtimeWindows.length === 0 ? (
                  ...
                  runtimeWindows.map((windowRecord) => (
                    <div key={windowRecord.runtimeLabel} className="rounded-xl border border-white/10 bg-black/20 p-3 text-sm text-forge-mist">
                      ...
                      <div className="mt-1 font-mono text-[11px]">{windowRecord.runtimeLabel}</div>
                      <div className="mt-1 text-[11px]">route {windowRecord.currentRoute} · monitor {windowRecord.monitorId ?? "unknown"}</div>
                      {windowRecord.bounds ? <div className="mt-1 text-[11px]">{windowRecord.bounds.x},{windowRecord.bounds.y} · {windowRecord.bounds.width}x{windowRecord.bounds.height}</div> : null}
                    </div>
                  ))
                )}
              </div>
