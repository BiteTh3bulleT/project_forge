...
export function BackupPage() {
  ...
  const [dirs, setDirs] = useState<{ backupDir: string; exportDir: string; kinds: string[] }>({ backupDir: "", exportDir: "", kinds: [] });
  ...
  async function refresh() {
    try {
      ...
      setDirs({ backupDir: r.backupDir, exportDir: r.exportDir, kinds: r.knownKinds });
      setErr(null);
    } catch (e) {
    ...
    <div className="space-y-6">
      <Panel title="Backup & export" subtitle="Portable bundles, not raw sqlite dumps. Version tags are operator-owned." actions={<GhostButton onClick={() => void refresh()}>Refresh</GhostButton>}>
        ...
        <div className="mt-3 space-y-1 text-xs text-forge-mist">
          <div>
            Backups dir: <span className="font-mono text-forge-ash">{dirs.backupDir || "—"}</span>
          </div>
          <div>
            Exports dir: <span className="font-mono text-forge-ash">{dirs.exportDir || "—"}</span>
          </div>
        </div>
      </Panel>

      <Panel title="Create bundle" subtitle="Kinds are validated server-side against KnownKinds.">
        <label className="text-xs text-forge-mist">
          Kind
          <select className="forge-input mt-1" value={kind} onChange={(e) => setKind(e.target.value)}>
            {dirs.kinds.map((k) => (
        ...
        <label className="mt-3 block text-xs text-forge-mist">
          Label
          <input className="forge-input mt-1" value={label} onChange={(e) => setLabel(e.target.value)} placeholder="weekly snapshot" />
        </label>
        <label className="mt-3 block text-xs text-forge-mist">
          Version tag
          <input className="forge-input mt-1" value={versionTag} onChange={(e) => setVersionTag(e.target.value)} placeholder="v0.5.0-2026-04-15" />
        </label>
        <div className="mt-4">
          <PrimaryButton
            onClick={async () => {
              const r = await api.backup.createBundle({ kind, label, versionTag, notes: "", sourceVersion: "" });
              setStatus("Bundle created.");
              setRestorePath(String((r.bundle as Record<string, unknown>).filePath ?? ""));
              await refresh();
            }}
          >
            Create
      ...
      <Panel title="Restore bundle" subtitle="Conservative merge. Always dry-run against an unknown file first.">
        ...
        <div className="mt-3">
          <PrimaryButton
            onClick={async () => {
              const r = await api.backup.restore({ filePath: restorePath, sections: [], dryRun: restoreDry });
          ...
          {bundles.map((b) => (
            <div key={b.id} className="flex flex-wrap items-start justify-between gap-2 rounded border border-white/10 bg-forge-slate/20 p-3 text-xs text-forge-mist">
              <div>
                ...
                <div className="mt-1 max-w-[640px] break-all font-mono text-[10px]">{b.filePath}</div>
              </div>
              <GhostButton
                onClick={async () => {
