...
function parseInput(raw: string): Record<string, unknown> {
  ...
  for (const line of trimmed.split(/\r?\n/)) {
    ...
    if (splitAt < 1)
      throw new Error(`Request detail is missing a colon: ${clean}`);
    next[clean.slice(0, splitAt).trim()] = parseReadableValue(
      clean.slice(splitAt + 1).trim(),
    );
  }
  return next;
}
...
export function ToolGatewayPage() {
  ...
  const [statusDraftById, setStatusDraftById] = useState<
    Record<string, ToolCapabilityStatus>
  >({});
  ...
  const selectedTool = useMemo(
    () => tools.find((x) => x.id === toolId) ?? null,
    [toolId, tools],
  );
  const selectedLane = useMemo(
    () => lanes.find((row) => row.id === laneId) ?? null,
    [laneId, lanes],
  );
  const selectedCapability = useMemo(() => {
    if (!selectedTool?.capabilityId) return null;
    return (
      capabilities.find((row) => row.id === selectedTool.capabilityId) ?? null
    );
  ...
  const canExecuteLocally = useMemo(() => {
    ...
    const capabilityAllowed =
      capabilityStatus === "active" || capabilityStatus === "approval_only";
    return Boolean(
      selectedTool &&
      selectedLane &&
  ...
  const preflightGates = useMemo(
    () => [
      ...
      { label: "and lane is enabled", pass: Boolean(selectedLane?.enabled) },
      {
        ...
        pass: (() => {
          const status =
            selectedCapability?.status ?? selectedTool?.capabilityStatus ?? "";
  ...
  async function refresh() {
    try {
      ...
      const capabilityRows = (
        Array.isArray(c.capabilities) ? c.capabilities : []
      ) as CapabilityRow[];
      ...
      const nextToolId = toolRows.some((row) => row.id === toolId)
        ? toolId
        : (toolRows[0]?.id ?? "");
      setToolId(nextToolId);
      const preferredLane =
        laneRows.find((row) => row.id === nextToolId && row.enabled) ??
        laneRows.find((row) => row.enabled) ??
        laneRows[0] ??
        null;
    ...
    <div className="forge-ops-board space-y-5">
      ...
      >
        ...
        <div className="mt-3 grid gap-2 text-xs text-forge-mist md:grid-cols-2">
          <div>
            Workspace root:{" "}
            <span className="font-mono text-forge-ash">{workspace || "—"}</span>
          </div>
          <label className="justify-self-start md:justify-self-end">
            Filter status
            <select
              className="forge-input ml-2"
              value={statusFilter}
            ...
            <GhostButton className="ml-2" onClick={() => void refresh()}>
              Apply
            </GhostButton>
          </label>
        </div>
      ...
      <Panel
        title="Invoke Action"
        subtitle="Typed request contract: tool + lane + risk + execution level + scoped targets + request details."
      >
        <div className="space-y-3">
          <FoldSection
            title="1) Target and lane"
            subtitle="Choose exactly what you want to invoke."
            defaultOpen
          >
          ...
          <FoldSection
            title="2) Risk and execution"
            subtitle="Explicitly set intent level before running."
            defaultOpen
          >
          ...
          <FoldSection
            title="3) Scope and details"
            subtitle="Constrain paths and provide named request details."
          >
            <label className="block text-xs text-forge-mist">
              Paths (comma-separated, relative to workspace or absolute if
              policy allows)
              <input
          ...
          >
            ...
            <div className="mt-2 text-[11px] text-forge-mist">
              ...
              <span className="text-forge-ash">
                {selectedCapability?.id ?? selectedTool?.capabilityId ?? "—"}
              </span>{" "}
              · status{" "}
              <span className="text-forge-ash">
                {selectedCapability?.status ??
                  selectedTool?.capabilityStatus ??
                  "unknown"}
              </span>
            </div>
          </FoldSection>
          ...
          <PrimaryButton
            ...
            onClick={async () => {
              try {
                ...
                setLast(out);
                setStatus(
                  `Gateway ${String(out.status ?? "ok")} · ${String(out.policyOutcome ?? "")}`.trim(),
                );
                await refresh();
              } catch (e) {
                setErr(e instanceof Error ? e.message : String(e));
        ...
        {last ? <GatewayResultSummary result={last} /> : null}
      </Panel>

      <Panel
        title="Registered Tools"
        subtitle="Bounded typed capabilities exposed by the gateway."
      >
        <div className="space-y-2">
          {tools.map((t) => (
            <div
              key={t.id}
              className="rounded border border-forge-platinum/10 bg-forge-slate/20 px-3 py-2 text-xs text-forge-mist"
            >
              <div className="font-mono text-forge-ash">{t.id}</div>
              <div className="mt-1">
                {t.domain}.{t.action} · {t.riskClass} · {t.executionLevel} ·{" "}
                {t.writeIntent ? "write" : "read"}
              </div>
              <div className="mt-1 text-[11px]">
                capability {t.capabilityId ?? "—"} · status{" "}
                {t.capabilityStatus ?? "—"} · risk {t.capabilityRisk ?? "—"} ·
                adapter {t.adapterId ?? "—"}
              </div>
              <div className="mt-1 text-[11px] text-forge-mist/90">
                status reason:{" "}
                {capabilityStatusReason(t.capabilityStatus, t.adapterId)}
              </div>
      ...
      <Panel
        ...
        subtitle="Kernel capability taxonomy, status, risk, and autonomy eligibility."
      >
        <div className="space-y-2">
      ...
      <Panel
        ...
        subtitle="Policy outcomes, approvals, and execution results from gateway history."
      >
        <div className="space-y-2">
          {invs.length === 0 ? (
            <div className="text-sm text-forge-mist">
              No invocations in this filter.
            </div>
          ) : null}
          {invs.map((row) => (
            <button
            ...
            >
              <div className="font-mono text-[11px] text-forge-ash">
                #{row.id} · {row.toolId} · {row.status} · {row.policyOutcome}
              </div>
              <div className="mt-1">
                {formatTime(row.createdAtMs)} · lane {row.laneId ?? "—"} · risk{" "}
                {row.riskClass} · {row.executionLevel}
              </div>
              <div className="mt-1">
                {row.deniedReason || row.correlationId}
              </div>
              {row.status === "unsupported" ? (
                <div className="mt-1 text-[11px] text-forge-mist/90">
                  Unsupported reason:{" "}
                  {row.deniedReason ||
                    "Capability is deferred or not implemented."}
                </div>
              ) : null}
              {row.jobId ? (
                <div className="mt-1 text-[11px]">Job: {row.jobId}</div>
              ) : null}
              {typeof row.approvalRequestId === "number" ? (
                <div className="mt-1 text-[11px]">
                  Approval request: {row.approvalRequestId}
                </div>
              ) : null}
            </button>
          ))}
      ...
      <div className="grid gap-1 md:grid-cols-2">
        {rows
          ...
          .map(([label, value]) => (
            <div
              key={label}
              className="flex items-start justify-between gap-3 border-b border-forge-platinum/5 pb-1"
            >
...
const CAPABILITY_STATUS_OPTIONS: ToolCapabilityStatus[] = [
  "active",
  "approval_only",
  "disabled",
  "stubbed",
...
function capabilityStatusReason(
  status?: ToolCapabilityStatus,
  adapterId?: string,
) {
  switch (status) {
    ...
    case "stubbed":
      if (!adapterId)
        return "Capability is registered but implementation is incomplete and currently unsupported.";
  ...
  return (
    <div className="flex items-center justify-between gap-3 border-b border-forge-platinum/5 pb-1 last:border-b-0 last:pb-0">
      ...
      <div
        className={
          props.pass
            ? "text-[11px] font-semibold text-forge-electric"
            : "text-[11px] font-semibold text-forge-emberSoft"
        }
      >
