...
export function LineagePage() {
  ...
  const loadRecent = useCallback(async () => {
    try {
      ...
      if (fromUrl) {
        ...
        setJobId((prev) => (prev.trim() ? prev : res.jobs.length > 0 ? res.jobs[0].id : ""));
      }
      setErr(null);
    } catch (e) {
  ...
  return (
    <div className="space-y-6">
      <Panel title="Lineage" subtitle="Retry/replay chains with explicit parent-child relationships and change summaries." actions={<GhostButton onClick={() => void loadRecent()}>Refresh</GhostButton>}>
        ...
        <div className="grid gap-3 md:grid-cols-3">
          <div className="md:col-span-2">
            <label className="text-xs font-semibold tracking-wide text-forge-mist">Job id</label>
            <input
          ...
          <div className="md:col-span-1">
            <label className="text-xs font-semibold tracking-wide text-forge-mist">Retry query override (optional)</label>
            <input aria-label="Retry query override" className="forge-input mt-1" value={retryQuery} onChange={(e) => setRetryQuery(e.target.value)} />
          </div>
        </div>
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          <GhostButton onClick={() => void loadLineage(jobId)}>Load Lineage</GhostButton>
        </div>
      </Panel>

      <Panel title="Recent Jobs" subtitle="Pick an origin run to inspect and extend lineage.">
        {recentJobs.length === 0 ? (
          ...
          <div className="space-y-2">
            {recentJobs.map((j) => (
              ...
              >
                ...
                <div className="mt-1 text-xs text-forge-mist">{j.id} | {j.status} | {j.targetAdapter} | {formatTime(j.createdAtMs)}</div>
              </button>
            ))}
          </div>
        )}
      ...
      <Panel title="Lineage Detail" subtitle="Parent/child edges with stored change summaries for replayability and debugging.">
        {!lineage ? (
