...
export function AutomationPage() {
  ...
  const [runResult, setRunResult] = useState<Record<string, unknown> | null>(
    null,
  );
  ...
  return (
    <div className="forge-ops-board space-y-5">
      <header className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <div className="forge-ops-label">Automation Control</div>
          <h1 className="mt-2 text-2xl font-semibold tracking-normal text-forge-ash sm:text-3xl">
            Automation rule board
          </h1>
          <p className="mt-1 max-w-3xl text-sm leading-6 text-forge-mist/75">
            Bounded trigger, condition, action contracts with dry-run preview
            and immutable run history.
          </p>
        </div>
        ...
        <div className="grid gap-3 md:grid-cols-3">
          <div>
            ...
            <input
              ...
              placeholder="Queue review after import"
            />
          </div>
          <div>
            <label className="text-xs font-semibold tracking-wide text-forge-mist">
            ...
            <input
              ...
              placeholder="import.execution.created"
            />
          </div>
                ...
                <button
                  ...
                  className={[
                    ...
                    selectedRuleId === r.id
                      ...
                      : "border-white/10 bg-black/20 hover:border-forge-ember/35",
                  ].join(" ")}
                >
                  ...
                  <div className="mt-1 text-xs text-forge-mist">
                    #{r.id} · {r.trigger} · enabled {String(r.enabled)} ·
                    dry-run default {String(r.dryRunDefault)}
                  </div>
                  <div className="mt-1 text-[11px] text-forge-mist">
                    updated {formatTime(r.updatedAtMs)}
                  </div>
                </button>
              ))}
            </div>
            ...
            <div className="space-y-2">
              {history.map((h) => (
                ...
                >
                  ...
                  <div className="mt-1">
                    trigger {h.trigger} · matched {String(h.matched)} · dry-run{" "}
                    {String(h.dryRun)}
                  </div>
                  <div className="mt-1">{h.message}</div>
                  <div className="mt-1 text-[11px]">
                    {formatTime(h.createdAtMs)}
                  </div>
                </div>
              ))}
            </div>
      ...
      >
        {!runResult ? (
          <div className="text-sm text-forge-mist">
            Run a rule to inspect the latest result.
          </div>
        ) : (
...
function ReadableMapInput(props: {
  label: string;
  value: string;
  ...
  return (
    <div>
      ...
      <textarea
        className="forge-input mt-1 min-h-[130px] font-mono text-[12px]"
        value={props.value}
